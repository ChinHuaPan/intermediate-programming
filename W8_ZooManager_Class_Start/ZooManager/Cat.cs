...
namespace ZooManager
{
    public class Cat : Animal
    {
        ...
        public bool Flee()
        {
            if (Game.Seek(location.x, location.y, Direction.up, "elephant"))
            {
                if (Game.Retreat(this, Direction.down)) return true;
            }
            if (Game.Seek(location.x, location.y, Direction.down, "elephant"))
            {
                if (Game.Retreat(this, Direction.up)) return true;
            }
            if (Game.Seek(location.x, location.y, Direction.left, "elephant"))
            {
                if (Game.Retreat(this, Direction.right)) return true;
            }
            if (Game.Seek(location.x, location.y, Direction.right, "elephant"))
            {
